...
function ca(){function b(m){const l=d;c=a=0;d=new Map;l.forEach(n=>{try{n(m)}catch(k){console.error(k)}});this.gb();e&&e.Eb()}let a=0,c=0,d=new Map,e=null,g=null;this.requestAnimationFrame=function(m){a||(a=requestAnimationFrame(b.bind(this)));const l=++c;d.set(l,m);return l};this.cancelAnimationFrame=function(m){d.delete(m);a&&0==d.size&&(cancelAnimationFrame(a),a=0)};this.Cb=function(m){g&&(document.body.remove(g),g=null);m||(g=document.createElement("div"),g.style.backgroundColor="black",g.style.position=
"fixed",g.style.right=0,g.style.top=0,g.style.color="white",g.style.padding="4px",g.innerHTML="RIVE FPS",m=function(l){g.innerHTML="RIVE FPS "+l.toFixed(1)},document.body.appendChild(g));e=new function(){let l=0,n=0;this.Eb=function(){var k=performance.now();n?(++l,k-=n,1E3<k&&(m(1E3*l/k),l=n=0)):(n=k,l=0)}}};this.zb=function(){g&&(document.body.remove(g),g=null);e=null};this.gb=function(){}}
...
this.fb=[]},addLoader:function(c){this.fb.push(c)},loadContents:function(c,d){for(let e of this.fb)if(e.loadContents(c,d))return!0;return!1}})};var ma=Object.assign({},h),na="./this.program",oa="object"==typeof window,qa="function"==typeof importScripts,y="",ra,sa;
...
function Xa(b){if(void 0===b)return"_unknown";b=b.replace(/[^a-zA-Z0-9_]/g,"$");var a=b.charCodeAt(0);return 48<=a&&57>=a?`_${b}`:b}function Ya(b,a){b=Xa(b);return{[b]:function(){return a.apply(this,arguments)}}[b]}function Za(){this.qa=[void 0];this.cb=[]}var Q=new Za,$a=void 0;function R(b){throw new $a(b);}
var S=b=>{b||R("Cannot use deleted val. handle = "+b);return Q.get(b).value},ab=b=>{switch(b){case void 0:return 1;case null:return 2;case !0:return 3;case !1:return 4;default:return Q.tb({jb:1,value:b})}};
...
function rb(b,a,c){if(a===c)return b;if(void 0===c.fa)return null;b=rb(b,a,c.fa);return null===b?null:c.Ab(b)}var xb={};function yb(b,a){a=ib(b,a);return hb[a]}var zb=void 0;function Ab(b){throw new zb(b);}function Bb(b,a){a.ea&&a.da||Ab("makeClassHandle requires ptr and ptrType");!!a.la!==!!a.ia&&Ab("Both smartPtrType and smartPtr must be specified");a.count={value:1};return Cb(Object.create(b,{ba:{value:a}}))}
...
this.Wb){case 0:a.ba.la===this?c=a.ba.ia:R(`Cannot convert argument of type ${a.ba.la?a.ba.la.name:a.ba.ea.name} to parameter type ${this.name}`);break;case 1:c=a.ba.ia;break;case 2:if(a.ba.la===this)c=a.ba.ia;else{var d=a.clone();c=this.Sb(c,ab(function(){d["delete"]()}));null!==b&&b.push(this.ra,c)}break;default:R("Unsupporting sharing policy")}return c}
...
function Vb(b,a,c,d){this.name=b;this.ca=a;this.Oa=c;this.Da=d;this.Ea=!1;this.ra=this.Sb=this.Pa=this.ib=this.Wb=this.Rb=void 0;void 0!==a.fa?this.toWireType=Tb:(this.toWireType=d?Rb:Ub,this.ka=null)}function Wb(b,a,c){h.hasOwnProperty(b)||Ab("Replacing nonexistant public symbol");void 0!==h[b].ga&&void 0!==c?h[b].ga[c]=a:(h[b]=a,h[b].va=c)}
...
function bc(b,a,c){b instanceof Object||R(`${c} with invalid "this": ${b}`);b instanceof a.ca.constructor||R(`${c} incompatible with "this" of type ${b.constructor.name}`);b.ba.da||R(`cannot call emscripten binding method ${c} on deleted object`);return Qb(b.ba.da,b.ba.ea.ca,a.ca)}function cc(b){b>=Q.Ya&&0===--Q.get(b).jb&&Q.wb(b)}
...
100|0,2),"%d":f=>g(f.Ra,2),"%e":f=>e(f.Ra,2," "),"%g":f=>n(f).toString().substring(2),"%G":f=>n(f),"%H":f=>g(f.Ia,2),"%I":f=>{f=f.Ia;0==f?f=12:12<f&&(f-=12);return g(f,2)},"%j":f=>{for(var r=0,q=0;q<=f.Ja-1;r+=(Cc(f.ua+1900)?Dc:Ec)[q++]);return g(f.Ra+r,3)},"%m":f=>g(f.Ja+1,2),"%M":f=>g(f.Yb,2),"%n":()=>"\n","%p":f=>0<=f.Ia&&12>f.Ia?"AM":"PM","%S":f=>g(f.Zb,2),"%t":()=>"\t","%u":f=>f.na||7,"%U":f=>g(Math.floor((f.ta+7-f.na)/7),2),"%V":f=>{var r=Math.floor((f.ta+7-(f.na+6)%7)/7);2>=(f.na+371-f.ta-
2)%7&&r++;if(r)53==r&&(q=(f.na+371-f.ta)%7,4==q||3==q&&Cc(f.ua)||(r=1));else{r=52;var q=(f.na+7-f.ta-1)%7;(4==q||5==q&&Cc(f.ua%400-1))&&r++}return g(r,2)},"%w":f=>f.na,"%W":f=>g(Math.floor((f.ta+7-(f.na+6)%7)/7),2),"%y":f=>(f.ua+1900).toString().substring(2),"%Y":f=>f.ua+1900,"%z":f=>{f=f.Xb;var r=0<=f;f=Math.abs(f)/60;return(r?"+":"-")+String("0000"+(f/60*100+f%60)).slice(-4)},"%Z":f=>f.$b,"%%":()=>"%"};c=c.replace(/%%/g,"\x00\x00");for(t in k)c.includes(t)&&(c=c.replace(new RegExp(t,"g"),k[t](d)));
c=c.replace(/\0\0/g,"%");t=Fc(c);if(t.length>a)return 0;Ba.set(t,b);return t.length-1};Object.assign(Za.prototype,{get(b){return this.qa[b]},has(b){return void 0!==this.qa[b]},tb(b){var a=this.cb.pop()||this.qa.length;this.qa[a]=b;return a},wb(b){this.qa[b]=void 0;this.cb.push(b)}});$a=h.BindingError=class extends Error{constructor(b){super(b);this.name="BindingError"}};Q.qa.push({value:void 0},{value:null},{value:!0},{value:!1});Q.Ya=Q.qa.length;
...
delete Db[b];var c=a.Pa,d=a.ra,e=a.bb,g=e.map(m=>m.Jb).concat(e.map(m=>m.Ub));V([b],g,m=>{var l={};e.forEach((n,k)=>{var t=m[k],v=n.Hb,x=n.Ib,f=m[k+e.length],r=n.Tb,q=n.Vb;l[n.Db]={read:G=>t.fromWireType(v(x,G)),write:(G,D)=>{var u=[];r(q,G,f.toWireType(u,D));Eb(u)}}});return[{name:a.name,fromWireType:function(n){var k={},t;for(t in l)k[t]=l[t].read(n);d(n);return k},toWireType:function(n,k){for(var t in l)if(!(t in k))throw new TypeError(`Missing field: "${t}"`);var v=c();for(t in l)l[t].write(v,
...
Pa:Z(c,d),ra:Z(e,g),bb:[]}},P:function(b,a,c,d,e,g,m,l,n,k){Db[b].bb.push({Db:U(a),Jb:c,Hb:Z(d,e),Ib:g,Ub:m,Tb:Z(l,n),Vb:k})},L:function(b,a){a=U(a);Ib(b,{Mb:!0,name:a,argPackAdvance:0,fromWireType:function(){},toWireType:function(){}})},v:function(b,a,c){b=S(b);a=nb(a,"emval::as");var d=[],e=ab(d);P[c>>2]=e;return a.toWireType(d,b)},w:function(b,a,c,d,e){b=uc[b];a=S(a);c=tc(c);var g=[];P[d>>2]=ab(g);return b(a,c,g,e)},i:function(b,a,c,d){b=uc[b];a=S(a);c=tc(c);b(a,c,null,d)},c:cc,h:function(b,a){var c=
wc(b,a),d=c[0];a=d.name+"_$"+c.slice(1).map(function(m){return m.name}).join("_")+"$";var e=xc[a];if(void 0!==e)return e;var g=Array(b-1);e=vc((m,l,n,k)=>{for(var t=0,v=0;v<b-1;++v)g[v]=c[v+1].readValueFromPointer(k+t),t+=c[v+1].argPackAdvance;m=m[l].apply(m,g);for(v=0;v<b-1;++v)c[v+1].yb&&c[v+1].yb(g[v]);if(!d.Mb)return d.toWireType(n,m)});return xc[a]=e},t:function(b){b=tc(b);return ab(h[b])},M:function(b,a){b=S(b);a=S(a);return ab(b[a])},n:function(b){4<b&&(Q.get(b).jb+=1)},o:function(b){return ab(tc(b))},
